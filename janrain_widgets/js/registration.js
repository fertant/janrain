...
janrain.settings.capture.beforeJanrainCaptureWidgetOnLoad.push(
  function () {
    jQuery.ajax({
      url: janrain.plex.drupalPath() + 'rest/session/token',
...
janrain.plex.login = function(oauthCode) {
  ...
  jQuery.ajax({
    url: janrain.plex.drupalPath() + 'janrain/registration/code?_format=json',
    type:'post',
    cache: false,
...
janrain.plex.refreshToken = function() {
  ...
  jQuery.ajax({
    ...
    success: function (drupalToken) {
      ...
      jQuery.ajax({
        url: janrain.plex.drupalPath() + 'janrain/registration/session_token?_format=json',
        type:'post',
        xhrFields:{withCredentials:true},
